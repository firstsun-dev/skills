...
- [x] Ad-hoc: Fixed leftover stale `actions/checkout@v4`/`pnpm/action-setup@v4` in `github-actions/references/unified-workflow.md`'s app-a/app-b jobs (missed by an earlier partial edit). Added `custom/develop/firstsun-dev-conventions/references/git-workflow.md` (GitHub Flow branch model, Conventional Commits + husky/commitlint enforcement — kept generic/portable, not firstsun-dev-specific) and a Node 24 LTS line in its `cicd.md`. Added the `registry.firstsun.org/firstsun-dev` private container registry + `DOCKER_AUTH_CONFIG` org-secret auth step to `firstsun-project-init` instead, since that registry and its secrets are firstsun-dev-org-specific, not portable convention. `/validate-skills` passed clean on all touched skills.
